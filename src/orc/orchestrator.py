...
from .gateway import get_spreadsheet, configure_rate_limiting, update_active_workers
from .gateway.leader import try_acquire_leadership, release_leadership
...
class Orchestrator:
        ...
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if 'entries' in info_dict:
                    ...
                    for entry in entries:
                        ...
                        if len(batch) >= batch_size:
                            ...
                            logger.info(f"Batch de {len(batch)} tasks adicionadas ({tasks_created} total)")
                            batch = []
                ...
                if batch:
                    ...
                    append_rows(self.task_table.tasks_ws, batch)
                    tasks_created += len(batch)
                    logger.info(f"Batch final de {len(batch)} tasks adicionadas ({tasks_created} total)")
    ...
    def process_next_task(self, callback: Callable[[str], None]) -> bool:
        ...
        try:
            ...
            if tasks_created > 0:
                logger.info(f"Source processada: {tasks_created} tasks criadas. Processando próxima iteração...")
