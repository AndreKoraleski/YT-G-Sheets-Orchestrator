...
from orc.gateway.leader import ELECTION_HEADER, ELECTION_SHEET_NAME, _ensure_election_worksheet
...
class TestTryAcquireLeadership:
    ...
    def test_acquire_new_election(self, mock_append, mock_select, mock_mapping, mock_ensure):
        ...
        result = try_acquire_leadership(Mock(), "test_election", "worker1", ttl_seconds=60)
    ...
    def test_acquire_expired_leadership(
        self, mock_time, mock_get_row, mock_update, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.select_first_by_columns")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_already_active(
        self, mock_time, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.get_row")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_exception(
        self, mock_time, mock_get_row, mock_update, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.get_row")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_renew_same_worker(
        self, mock_time, mock_get_row, mock_update, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.get_row")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_invalid_expiration(
        self, mock_time, mock_get_row, mock_update, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.select_first_by_columns")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_inactive_status(
        self, mock_time, mock_select, mock_mapping, mock_ensure
    ):
    ...
    @patch("orc.gateway.leader.get_row")
    @patch("orc.gateway.leader.time")
    def test_acquire_leadership_check_row_returns_different_leader(
        self, mock_time, mock_get_row, mock_update, mock_select, mock_mapping, mock_ensure
    ):
...
class TestReleaseLeadership:
    ...
    def test_release_leadership_election_not_found(self, mock_select, mock_mapping, mock_ensure):
        ...
        # Não deve lançar exceção
        release_leadership(Mock(), "nonexistent", "worker1")

    @patch("orc.gateway.leader._ensure_election_worksheet")
    @patch("orc.gateway.leader.get_header_mapping")
    @patch("orc.gateway.leader.select_first_by_columns")
    @patch("orc.gateway.leader.update_row")
    def test_release_leadership_exception_silenced(
        self, mock_update, mock_select, mock_mapping, mock_ensure
    ):
        ...
        mock_ws = Mock()
        mock_ensure.return_value = mock_ws
        mock_mapping.return_value = {"Nome da Eleição": 0, "ID do Líder": 1, "Status": 4}
